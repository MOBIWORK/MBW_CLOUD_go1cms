...
class PageSection(Document):
    ...
    def validate(self):
        ...
        if self.section_type == 'Custom Section' and self.content_type == 'Dynamic':
            ...
            fields = ["web_image", "mobile_app_image", "mobile_image",
                      "title", "sub_title", "button_text", "redirect_url"]
            self.field_list = json.dumps(fields)
    ...
    def on_update(self):
        ...
        pages = frappe.db.sql('''select distinct parent from `tabMobile Page Section` where section = %(section)s and parenttype="Web Page Builder"''', {
                              'section': self.name}, as_dict=1)
        if pages:
            for item in pages:
    ...
    def section_data(self, customer=None, add_info=None, store_business=None):
        ...
        if self.section_type == 'Predefined Section' and not self.is_login_required:
            ...
            if check_domain("multi_store") and not store_business:
                multi_store_business = frappe.request.cookies.get(
                    'selected_store')
                if not multi_store_business:
                    all_locations = frappe.db.get_all(
                        "Business", fields=['name', 'restaurant_name'], order_by="is_default desc")
                    if all_locations:
                ...
                if multi_store_business:
                    slider_cond = ' and business = "{0}"'.format(
                        multi_store_business)
            if check_domain("multi_store"):
                if store_business:
                    slider_cond = ' and business = "{0}"'.format(
                        store_business)
            json_obj['data'] = frappe.db.sql(
                '''select business,mobile_app_image,mobile_app_videoyoutube_id,mobile_image,mobile_videoyoutube_id,redirect_url,slider_type,upload_video_for_mobile,upload_video_for_mobile_app,upload_video_for_web,video_type,web_image,web_videoyoutube_id from `tabSlider` where published = 1 {cond} order by display_order'''.format(cond=slider_cond), as_dict=1)
            ...
            if self.content_type == 'Static':
                if self.reference_document == 'Product Category':
                    json_obj['route'] = frappe.db.get_value(
                        self.reference_document, self.reference_name, "route")
                ...
                if self.reference_document == 'Product Category' and self.dynamic_data == 0:
                    ...
                    if len(json_obj['data']) > 0 and self.reference_name:
                        ...
                        if self.reference_document == 'Publisher':
                            field = 'name'
                        if field:
                            json_obj['title'] = frappe.db.get_value(
                                self.reference_document, self.reference_name, field)
                            if self.reference_document == 'Product Category':
                                json_obj['route'] = frappe.db.get_value(
                                    self.reference_document, self.reference_name, "route")
            ...
            if self.reference_document == 'Custom Query':
                ...
                for item in data:
                    ...
                    item['name'] = item.get(
                        'tab_item').lower().replace(' ', '_')
                    query_item = frappe.db.get_value(
                        self.reference_document, item.get('tab_item'), 'query')
                    query = '''{query} limit {limit}'''.format(
                        query=query_item, limit=no_of_records)
                    result = frappe.db.sql(
                        query, {"business": self.business}, as_dict=1)
            ...
            if 'erp_ecommerce_business_store' in frappe.get_installed_apps():
                ...
                json_obj['data'] = get_list_data(
                    self, customer=None, add_info=None, store_business=None)
...
def get_data_source(query, dt=None, no_of_records=0, login_required=0, customer=None, user=None, business=None,
    ...
    if check_domain("multi_store") and not store_business:
        ...
        if multi_store_business:
            query = query.replace(
                'where p.is_active', 'where  p.restaurant = "{0}" AND p.is_active '.format(multi_store_business))
            query = query.replace('where parent_product_category is null and',
                                  'where  parent_product_category is null and business = "{0}" AND '.format(multi_store_business))
    if check_domain("multi_store"):
        if store_business:
            query = query.replace(
                'where p.is_active', 'where  p.restaurant = "{0}" AND p.is_active '.format(store_business))
            query = query.replace('where parent_product_category is null and',
                                  'where  parent_product_category is null and business = "{0}" AND '.format(store_business))
    if check_domain('saas'):
        domain = frappe.get_request_header('host')
        business = get_business_from_web_domain(domain)
        if business:
            query = query.replace(
                'where p.is_active', 'where  p.restaurant = "{0}" AND p.is_active '.format(business))
    ...
    if login_required:
        if not customer:
            customer = urllib.parse.unquote(frappe.request.cookies.get(
                'customer_id')) if frappe.request.cookies.get('customer_id') else None
        if not user:
    ...
    if check_domain('restaurant'):
        ...
        filters['distance'] = get_settings_value_from_domain(
            'Business Setting', 'nearby_distance', business=business)
        unit = get_settings_value_from_domain(
            'Business Setting', 'distance_unit', business=business)
        if unit != 'Miles':
        ...
        try:
            ...
            if not order_type:
                filters['order_type'] = frappe.request.cookies.get(
                    'order_type')
            order_date = frappe.request.cookies.get('order_date')
            if order_date:
                filters['today_date'] = getdate(
                    urllib.parse.unquote(order_date))
            ...
            pass
    try:
        result = frappe.db.sql('''{query}'''.format(
            query=query), filters, as_dict=1)
        ...
        ), "ecommerce_business_store.cms.doctype.page_section.page_section.get_data_source")
        return []
...
def get_recommended_products(query=None, dt=None, no_of_records=0, login_required=0, customer=None, user=None, business=None,
    ...
    if catalog_settings:
        ...
        if catalog_settings.enable_recommended_products:
            ...
            if customer:
                cond = " where o.customer='{customer}'""".format(
                    customer=customer)
                viewed_query = """select distinct product from `tabCustomer Viewed Product` where parent ='{customer}'""".format(
                    customer=customer)
                viewed_items = frappe.db.sql(viewed_query, as_dict=True)
                # frappe.log_error(viewed_items, "viewed_items")

                orderquery = """select MAX(i.item) as product from `tabOrder` o inner join `tabOrder Item` i ON i.parent=o.name  {cond}""".format(
                    cond=cond)
                order_items = frappe.db.sql(orderquery, as_dict=True)
                # frappe.log_error(order_items, "order_items")

                cartquery = """select i.product, i.price from `tabShopping Cart` o inner join `tabCart Items` i ON i.parent=o.name  {cond}""".format(
                    cond=cond)
                ...
                if not order_items:
                    order_items = []
            # frappe.log_error(order_items, "order_items")
            recommended_item_list = ",".join(
                ['"' + x.product + '"' for x in order_items if x.product])
            catquery = """select distinct category from `tabProduct Category Mapping`"""
            if recommended_item_list:
                catquery = """select distinct category from `tabProduct Category Mapping` where parent in ({lists})""".format(
                    lists=recommended_item_list)
            ...
            if category_list:
                ord_query = """select p.*,(select list_image from `tabProduct Image` where parent=p.name order by is_primary desc limit 1) as product_image from `tabProduct` p inner join `tabProduct Category Mapping` pc on pc.parent=p.name where pc.category in ({category_list}) and p.price >='{min_val}' and p.price <='{max_val}' limit 1,{no_of_records}""".format(
                    no_of_records=no_of_records, lists=recommended_item_list, category_list=category_list, min_val=min_val, max_val=max_val)
            else:
                ord_query = """select p.*,(select list_image from `tabProduct Image` where parent=p.name order by is_primary desc limit 1) as product_image from `tabProduct` p inner join `tabProduct Category Mapping` pc on pc.parent=p.name where  p.price >='{min_val}' and p.price <='{max_val}' limit 1,{no_of_records}""".format(
                    no_of_records=no_of_records, lists=recommended_item_list, category_list=category_list, min_val=min_val, max_val=max_val)
...
def get_dynamic_data_source(doc, customer=None, store_business=None):
    ...
    if not business:
        business = get_business_from_login()
    if doc.condition:
        condition = ' and {0}'.format(doc.condition)
    if doc.fetch_product:
        ...
        if doc.reference_document == 'Product Category':
            ...
            condition += ' and (child.category = "{0}" or child.category in ({1}))'.format(
                doc.reference_name, child_category)
            ...
            if not store_business:
                if not multi_store_business:
                    ...
                    multi_store_business = unquote(
                        frappe.request.cookies.get('selected_store'))
                if multi_store_business:
                    condition += "AND doc.restaurant = '{0}' ".format(
                        multi_store_business)
        if check_domain("multi_store"):
            if store_business:
                condition += "AND doc.restaurant = '{0}' ".format(
        ...
        if check_domain("multi_store") and doc.reference_document == 'Product Category':
            ...
            if not multi_store_business:
                ...
                multi_store_business = unquote(
                    frappe.request.cookies.get('selected_store'))
            if multi_store_business:
                condition += "AND doc.business = '{0}' ".format(
                    multi_store_business)
        if check_domain('saas'):
            domain = frappe.get_request_header('host')
            business = get_business_from_web_domain(domain)
        ...
        query = '''SELECT {fields} from `tab{doctype}` doc {condition} order by doc.{field} {sort_by} limit {limit}'''.format(
            fields=fields, doctype=doc.reference_document, condition=condition, field=(doc.sort_field or 'name'), sort_by=doc.sort_by, limit=doc.no_of_records)
        result = frappe.db.sql(query, as_dict=1)
    return result
...
def get_item_info(dt, dn):
    ...
    images = []
    if dt == 'Product':
        images = frappe.db.sql('''select list_image, detail_thumbnail as thumbnail from `tabProduct Image` where parent = %(parent)s order by idx''', {
                               'parent': dn}, as_dict=1)
    else:
        fields = filter(lambda x: x.fieldtype in [
                        'Attach', 'Attach Image'], meta.fields)
        for item in fields:
...
def update_page_sections():
    doc_list = frappe.db.sql(
        '''select name from `tabPage Section` where section_type <> "Banner" or (section_type = "Custom Section" and content_type = "Dynamic")''', as_dict=1)
    if doc_list:
...
def get_section_data(section, customer=None):
    section_data = frappe.db.sql('''select section, parent, parentfield from `tabMobile Page Section` where section = %(name)s''', {
                                 'name': section}, as_dict=1)
    data_source = None
    if section_data:
        path = 'data_source/{0}_{1}.json'.format(section_data[0].parent.lower().replace(
            ' ', '_'), ('web' if section_data[0].parentfield == 'web_section' else 'mobile'))
        ...
        if os.path.exists(file_path):
            with open(file_path) as f:
                ...
                data_source = next(
                    (x for x in data if x.get('section') == section), None)
                ...
                if data_source['dynamic_data'] == 1:
                    if data_source['section_type'] in ['Predefined Section', 'Custom Section', 'Lists', 'Tabs']:
                        doc = frappe.get_doc(
                            'Page Section', data_source['section'])
...
def generate_static_section(section):
    ...
    template = ""
    business = None
    product_template = frappe.db.get_value("Page Section", section, [
                                           "name", "business", "section_title", "web_template", "custom_css", "custom_js"], as_dict=True)
    if product_template:
        ...
        html_page = product_template.section_title.lower().replace(
            ' ', '-') + "-" + (product_template.name).lower().replace(' ', '-')
...
def generate_dynamic_section(section, view_type):
    ...
    product_template = frappe.db.get_value("Page Section", section, [
                                           "name", "section_title", "web_template", "mobile_view_template", "custom_css", "custom_js"], as_dict=True)
    # end
    if product_template:
        # context={}
    ...
    try:
        if category:
            lft, rgt = frappe.db.get_value(
                'Product Category', category, ['lft', 'rgt'])
...
def save_page_section_to_template(section_id):
    ...
    section_template.query_by_category = page_section.query_by_category
    section_template.category = page_section.category
