...
from go1_cms.go1_cms.doctype.xxx_form_script.xxx_form_script import get_form_script
...
def get_list_data(
	...
	if "search_text" in filters:
		# Lấy và xóa search_text khỏi filters
	...
	if view_type != "kanban":
		...
		if not custom_view and frappe.db.exists("CMS View Settings", default_view_filters):
			list_view_settings = frappe.get_doc("CMS View Settings", default_view_filters)
			columns = frappe.parse_json(list_view_settings.columns)
			rows = frappe.parse_json(list_view_settings.rows)
			is_default = False
   
		# if default_list_data(doctype).get("columns") :
		# 	if not custom_view or is_default :
		# 		rows = default_rows
	...
	if not is_default and custom_view_name:
		is_default = frappe.db.get_value("CMS View Settings", custom_view_name, "load_default_columns")
...
def get_quick_filters(doctype: str):
	...
	for field in fields:
		ops = field.options
		if field.fieldtype == "Select":
			print(">>>>>>>>>>>>>>>>>>>>>>>>>>> field.options", field.options)
	...
	# Nếu có điều kiện cụ thể cho một Doctype, bạn có thể thêm tại đây
	# if doctype == "xxx_yyy":
