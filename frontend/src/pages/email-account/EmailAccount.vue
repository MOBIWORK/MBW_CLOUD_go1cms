<template>
		...
		<ViewControls
			ref="viewControls"
			:modelValue="listData"
			@update:modelValue="listData = $event"
			:loadMore="loadMore"
			@update:loadMore="loadMore = $event"
			:resizeColumn="triggerResize"
			@update:resizeColumn="triggerResize = $event"
			:updatedPageCount="updatedPageCount"
			@update:updatedPageCount="updatedPageCount = $event"
		...
		<DataListView
			...
			:modelValue="listData.data.page_length_count"
			@update:modelValue="listData.data.page_length_count = $event"
			:list="listData"
		...
		<Modal
			:modelValue="showInfoModalData"
			@update:modelValue="showInfoModalData = $event"
			:list_field_value="list_field_value"
			:editMode="editMode"
			:doctype="'Email Account'"
			:quickEntry="showQuickEntryModal"
			@update:quickEntry="showQuickEntryModal = $event"
			@updateList="listData.reload()"
		></Modal>

		<QuickEntryModal :modelValue="showQuickEntryModal" @update:modelValue="showQuickEntryModal = $event" doctype="Email Account" />
...
import Modal from "@/components/Modals/EmailAccount_Modal.vue";
import DataListView from "@/components/ListViews/EmailAccount_ListView.vue";
...
const canCreate = can("Email Account", "create");

const breadcrumbs = [{ label: __("Email Account"), route: { name: "EmailAccount" } }];
