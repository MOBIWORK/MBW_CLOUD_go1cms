<template>
  <ListView :class="$attrs.class" :columns="columns" :rows="rows" :options="{
    ...
    <ListRows id="list-rows" v-if="rows && rows.length" doctype="CMS_JobOpening" class="relative">
      <ListRow v-for="row in rows" :key="row.name_web" v-slot="{ idx, column, item }" :row="row">
        <div v-if="column.key === 'jo_id'">
          <Tooltip :text="item">
            <div class="flex items-center gap-2 truncate text-base cursor-pointer"
              @click="() => navigateToJobOpeningDetail(row.name)">
        ...
        <div v-else-if="column.key === 'jo_application_deadline'">
          <Tooltip :text="item && formattedDateTooltip(item)">
        ...
        <div v-else-if="column.key === 'status'" class="truncate text-base">
          <div v-if="slaStatusColorMap.hasOwnProperty(item)">
            <Badge :variant="'subtle'" :theme="slaStatusColorMap[item]" size="md" :label="__(item)" @click="
              (event) =>
                emit('applyFilter', {
                  event,
                  idx,
        ...
        <div v-else-if="column.key === 'publish_to_career_page'" class="truncate text-base">
          <div v-if="item === 0">
            <Badge :variant="'subtle'" :theme="publishColorMap[item]" size="md" :label="__(publishLabelMap[item])" />
          </div>
  ...
  <ListFooter v-if="pageLengthCount" class="border-t py-2" v-model="pageLengthCount" :options="{
...
const navigateToJobOpeningDetail = (jobOpeningId) => {
  router.push({
    name: "ats_job_opening_view",
    params: { jobOpeningId: jobOpeningId },
  });
};
