...
import { getCalendarDates, monthList, handleSeconds, parseDate } from "./calendarUtils";
...
const emit = defineEmits(["updateActiveView", "updateCalendarState"]);
...
const parseEvents = computed(() => {
	return props.events.map((event) => {
		...
		const from_time = new Date(fromDate).toLocaleTimeString("en-GB", { hour12: false });
		const to_time = new Date(toDate).toLocaleTimeString("en-GB", { hour12: false });
		if (event.isFullDay) {
			return { ...rest, date };
		}
...
let currentYear = ref(props.currentYear);
let currentMonth = ref(props.currentMonth);
let currentDate = ref(new Date());

// Watch để cập nhật state khi `props` thay đổi từ cha
watch(
	() => props.currentYear,
	(newValue) => {
...
let week = ref(props.week || findCurrentWeek(currentDate.value));
...
function filterCurrentWeekDates() {
	...
	let differentMonthDates = currentWeekDates.filter((d) => d.getMonth() !== props.currentMonth);
