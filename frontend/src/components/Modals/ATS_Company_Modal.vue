...
let _address = ref({});
const context = __("company"); // Ngữ cảnh hiện tại
let _data = ref({
	name: "",
	company_id: "",
	company_name: "",
...
const canEdit = can("ATS_Company", "write");
...
const validateData = () => {
	if (!_data.value.company_name) {
		createToast({
			title: __("Error occurred"),
			text: __("Company Name is required."),
			icon: "x",
	...
	if (!_data.value.company_id) {
		createToast({
			title: __("Error occurred"),
			text: __("Company ID is required."),
			icon: "x",
			iconClasses: "text-red-600",
		});
		loading.value = false;
...
async function updateData() {
	...
	const nameChanged = old.name !== newOrg.company_name;
	delete old.name;
	delete newOrg.name;
...
async function callRenameDoc() {
	...
	try {
		const d = await call("frappe.client.rename_doc", {
			...
			new_name: _data.value.company_name,
		});
		loading.value = false;
		return d;
	} catch (error) {
...
const dialogOptions = computed(() => {
	let title = !editMode.value ? "Add Company" : "Edit Data";
	let size = "6xl";
