...
let _address = ref({});
const context = __("location"); // Ngữ cảnh hiện tại
let _data = ref({
    location_id: "",
    location_name: "",
    country_id: "",
...
const validateData = () => {
	if (!_data.value.location_name) {
		createToast({
			title: __("Error occurred"),
			text: __("Location Name is required."),
			icon: "x",
	...
	if (!_data.value.location_id) {
		createToast({
			title: __("Error occurred"),
			text: __("Location ID is required."),
			icon: "x",
			iconClasses: "text-red-600",
		});
		loading.value = false;
...
async function updateData() {
	...
	const nameChanged = old.name !== newOrg.location_name;
	delete old.name;
	delete newOrg.name;
...
async function callRenameDoc() {
	...
	try {
		const d = await call("frappe.client.rename_doc", {
			...
			new_name: _data.value.location_name,
		});
		loading.value = false;
		return d;
	} catch (error) {
...
async function callSetValue(values) {
	try {
		...
		const d = await call("frappe.client.set_value", {
			doctype: props.doctype,
			name: _data.value.location_name,
			fieldname: values,
		});
...
const dialogOptions = computed(() => {
	let title = !editMode.value ? "Add Location" : "Edit Data";
	let size = "6xl";
