...
let _address = ref({});
const context = __("level"); // Ngữ cảnh hiện tại
...
const validateData = () => {
	if (!_data.value.level_name) {
		createToast({
			title: __("Error occurred"),
			text: __("Level Name is required."),
			icon: "x",
	...
	if (!_data.value.level_id) {
		createToast({
			title: __("Error occurred"),
			text: __("Level ID is required."),
			icon: "x",
			iconClasses: "text-red-600",
		});
		loading.value = false;
...
async function updateData() {
	...
	const nameChanged = old.name !== newOrg.level_name;
	delete old.name;
	delete newOrg.name;
...
async function callRenameDoc() {
	...
	try {
		const d = await call("frappe.client.rename_doc", {
			...
			new_name: _data.value.level_name,
		});
		loading.value = false;
		return d;
	} catch (error) {
...
const dialogOptions = computed(() => {
	let title = !editMode.value ? "Add Level" : "Edit Data";
	let size = "6xl";
