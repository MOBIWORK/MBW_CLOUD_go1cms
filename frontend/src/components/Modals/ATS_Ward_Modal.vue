...
let _address = ref({});
const context = __("ward"); // Ngữ cảnh hiện tại
let _data = ref({
    ward_id: "",
    ward_name: "",
	district_id: "",
...
const validateData = () => {
	if (!_data.value.ward_name) {
		createToast({
			title: __("Error occurred"),
			text: __("Ward Name is required."),
			icon: "x",
	...
	if (!_data.value.ward_id) {
		createToast({
			title: __("Error occurred"),
			text: __("Ward ID is required."),
			icon: "x",
			iconClasses: "text-red-600",
		});
		loading.value = false;
...
async function updateData() {
	...
	const nameChanged = old.name !== newOrg.ward_name;
	delete old.name;
	delete newOrg.name;
...
async function callRenameDoc() {
	...
	try {
		const d = await call("frappe.client.rename_doc", {
			...
			new_name: _data.value.ward_name,
		});
		loading.value = false;
		return d;
	} catch (error) {
...
const dialogOptions = computed(() => {
	let title = !editMode.value ? "Add Ward" : "Edit Data";
	let size = "6xl";
