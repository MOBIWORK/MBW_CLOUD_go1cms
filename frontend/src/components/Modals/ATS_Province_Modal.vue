...
let _address = ref({});
const context = __("province"); // Ngữ cảnh hiện tại
let _data = ref({
    province_id: "",
    province_name: "",
    country_id: "", // dạng link
    cat_status: "Active",
    cat_order: "",
    cat_color: "",
    cat_icon: "",
...
const validateData = () => {
	if (!_data.value.province_name) {
		createToast({
			title: __("Error occurred"),
			text: __("Province Name is required."),
			icon: "x",
	...
	if (!_data.value.province_id) {
		createToast({
			title: __("Error occurred"),
			text: __("Province ID is required."),
			icon: "x",
			iconClasses: "text-red-600",
		});
		loading.value = false;
...
async function updateData() {
	...
	const nameChanged = old.name !== newOrg.province_name;
	delete old.name;
	delete newOrg.name;
...
async function callRenameDoc() {
	...
	try {
		const d = await call("frappe.client.rename_doc", {
			...
			new_name: _data.value.province_name,
		});
		loading.value = false;
		return d;
	} catch (error) {
...
async function callSetValue(values) {
	try {
		...
		const d = await call("frappe.client.set_value", {
			doctype: props.doctype,
			name: _data.value.province_name,
			fieldname: values,
		});
...
const dialogOptions = computed(() => {
	let title = !editMode.value ? "Add Province" : "Edit Data";
	let size = "6xl";
