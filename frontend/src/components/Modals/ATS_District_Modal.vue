...
let _address = ref({});
const context = __("district"); // Ngữ cảnh hiện tại

let _data = ref({
    district_id: "",
    district_name: "",
    province_id: "", // dạng link
    cat_status: "Active",
    cat_order: 0,
    cat_color: "",
    cat_icon: "",
...
const validateData = () => {
	if (!_data.value.district_name) {
		createToast({
			title: __("Error occurred"),
			text: __("District Name is required."),
			icon: "x",
	...
	if (!_data.value.district_id) {
		createToast({
			title: __("Error occurred"),
			text: __("District ID is required."),
			icon: "x",
			iconClasses: "text-red-600",
		});
		loading.value = false;
...
async function updateData() {
	...
	const nameChanged = old.name !== newOrg.district_name;
	delete old.name;
	delete newOrg.name;
...
async function callRenameDoc() {
	...
	try {
		const d = await call("frappe.client.rename_doc", {
			...
			new_name: _data.value.district_name,
		});
		loading.value = false;
		return d;
	} catch (error) {
...
async function callSetValue(values) {
	try {
		...
		const d = await call("frappe.client.set_value", {
			doctype: props.doctype,
			name: _data.value.district_name,
			fieldname: values,
		});
...
const dialogOptions = computed(() => {
	let title = !editMode.value ? "Add District" : "Edit Data";
	let size = "6xl";
