	...
	<Dialog v-model="show" :options="{ title: __('Bulk Edit') }">
		...
		<template #actions>
			<Button
				...
				:label="
					recordCount === 1
						? `${__('Update')} ${recordCount} ${__('Record')}`
						: `${__('Update')} ${recordCount} ${__('Records')}`
				"
			/>
		</template>
	</Dialog>
...
const fields = createResource({
  ...
  transform: (data) => {
    return data
      ...
      .map((field) => ({
        ...field,
        label: __(field.label), // Chuyển ngữ cho label
        options: field.options
          ? field.options
              .split("\n")
              .map((option) => __(option)) // Chuyển ngữ từng option
              .join("\n")
          : null,
      }));
  },
